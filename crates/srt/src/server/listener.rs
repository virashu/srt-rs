...
use crate::protocol::{
    constants::{FULL_ACK_INTERVAL, HANDSHAKE_MAGIC_CODE, MAX_PACKET_SIZE, RTT_INIT, RTT_VAR_INIT},
    packet::{
        Packet,
        PacketContent,
...
pub struct Connection {
    ...
    // /// Amount of packets received since last ack was **sent**
...
impl Connection {
    // pub fn establish_v5() -> Result<Self> {
    //     let mut buf = [0; 200];
    ...
    pub fn send(&self, content: PacketContent) -> Result<()> {
        let packet = self.pack(content)?.to_raw();
